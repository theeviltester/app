window.rhfs=window.rhfs||[];rhfs.push('./extensions/wikia/AdEngine/ghost/gw-12.4.4/t/dombuilder/t0.js');
function test(){ 
	var 
	...
	var tagCreator= new Y.Test.Case({ 
		...
		,"element closing should span scripts": function(){ 
			var 
			...
			ghostwriter( container, { 
				 script: { src: url } 
				,done: function(){ 
					var span= document.getElementById("close-test")
					test.resume(function(){
						Y.Assert.areEqual(0, span.innerHTML.indexOf("t1"))

						Y.Assert.areNotEqual(-1, span.innerHTML.indexOf("t1-1"))
					})
				}
			})
			this.wait(function(){ 
				Y.Assert.fail("never finished")
			}, 500)
		}
