window.rhfs=window.rhfs||[];rhfs.push('./extensions/wikia/AdEngine/ghost/gw-12.4.4/t/streamparser/t0.js');
...
function test(Y){
	...
	var 
		...
		,closingTokens= new Y.Test.Case({ 
			...
			,"parser should handle escaped script tags": testEscapedScriptTag 
		})
	; 
	Y.Test.Runner
		.add(tokenTest)
...
function testEscapedScriptTag(){ 
	var 
		html= unescape("%3CSCRIPT%20type%3D%22text/javascript%22%3E%0Afoo%3D1%0Adocument.write%28%27%3Cscript%20%5C%3E%0Afoo%3D2%3B%27%29%3B%20document.write%28%27%3C/script%5C%3E%20%5Cn%27%29%3B%20morebullshit%3C/SCRIPT%3E%20What?" )
		,started= 0
		,ended= 0 
		,parser= new ghostwriter.htmlstreamparser({ 
		 	start: function(){ 
				started++
				if(started > 1) 
					Y.Assert.fail("Too many START tags") 
			} 	
			,end: function(tag){ 
				if(++ended > 1) 
					Y.Assert.fail("Too many END tags " + tag) 
			} 
		})
	; 
	parser.parse(html)
