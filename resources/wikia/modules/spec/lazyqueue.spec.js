window.rhfs=window.rhfs||[];rhfs.push('./resources/wikia/modules/spec/lazyqueue.spec.js');
'use strict';
...
describe('LazyQueue', function () {
	...
	it('Queue callback is called for every item in array', function () {
		...
		expect(c.callback.calls.argsFor(0)[0]).toEqual('item0');
		expect(c.callback.calls.argsFor(1)[0]).toEqual('item1');
		expect(c.callback.calls.argsFor(2)[0]).toEqual('item2');
	});
	...
	it('Queue callback is called for each element pushed after start()', function () {
		...
		expect(c.callback.calls.count()).toEqual(2);
		expect(c.callback.calls.argsFor(0)[0]).toEqual('item0');
		expect(c.callback.calls.argsFor(1)[0]).toEqual('item1');
	});
